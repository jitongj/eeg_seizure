function fft_SeiDet_tensor(patient,sz_det_win,SamplingRate,rate)
fft_SeiDet_tensor= {};
for i =1: size(sz_det_win,2)
    FilteredChannelData1 = sz_det_win(i).window(1,:);
    ...
    time = size(y1,2);
    freq = size(y1,1);
    chan = size(sz_det_win(i).window,1);
    ...
    for n= 1:chan
        ...
        FilteredChannelData = sz_det_win(i).window(n,:);
    ...
    fft_SeiDet_tensor(i,1) = {tensor};
end               

filename=sprintf('fft_SeiDet_tensor%02d',patient);
save(filename,'fft_SeiDet_tensor');
end
