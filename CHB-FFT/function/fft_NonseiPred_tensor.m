function fft_NonseiPred_tensor(patient,nonsz_pred_win,SamplingRate,rate)
fft_NonseiPred_tensor = {};
for i =1: size(nonsz_pred_win,2)
    FilteredChannelData1 = nonsz_pred_win(i).window(1,:);
    ...
    time = size(y1,2);
    freq = size(y1,1);
    chan = size(nonsz_pred_win(i).window,1);
    ...
    for n= 1:chan
        ...
        FilteredChannelData = nonsz_pred_win(i).window(n,:);
    ...
    fft_NonseiPred_tensor(i,1) = {tensor};
end               

filename=sprintf('fft_NonseiPred_tensor%02d',patient);
save(filename,'fft_NonseiPred_tensor');
end
