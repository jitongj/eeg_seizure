function fft_NonseiDet_tensor(patient,nonsz_det_win,SamplingRate,rate)
fft_NonseiDet_tensor = {};
for i =1: size(nonsz_det_win,2)
    FilteredChannelData1 = nonsz_det_win(i).window(1,:);
    ...
    time = size(y1,2);
    freq = size(y1,1);
    chan = size(nonsz_det_win(i).window,1);
    ...
    for n= 1:chan
        ...
        FilteredChannelData = nonsz_det_win(i).window(n,:);
    ...
    fft_NonseiDet_tensor(i,1) = {tensor};
end               

filename=sprintf('fft_NonseiDet_tensor%02d',patient);
save(filename,'fft_NonseiDet_tensor');
end
